...
export const instructorRegister = async (
  ...
  for (const file of files) {
    ...
    if (file.originalname === 'profilePic') {
      ...
      const uploadedFile = `http://localhost:${process.env.PORT}/${file.filename}`;
      let data = { url: uploadedFile, key: 'profilePic', name: 'profilePic' };

      instructor.profilePic = data;
    } else {
      // uploadedFile = await cloudService.upload(file);
      const uploadedFile = `http://localhost:${process.env.PORT}/${file.filename}`;
      let data = { url: uploadedFile, name: file.originalname };

      instructor.certificates.push(data);
    }
  }
