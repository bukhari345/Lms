...
export function AdminSideNav() {
  ...
  return (
    <Card className="h-[calc(100vh-1rem)] w-full max-w-[20rem] p-4 m-3 shadow-xl shadow-blue-gray-900/5 bg-gradient-to-r from-purple-500 to-pink-500">
        ...
        {navItems.map((item, index) => (
          <Link to={`/admin/${item.path}`} key={index}>
            {selected === item.label ? (
              ...
              >
                <li className="cursor-pointer flex items-center gap-2 p-2.5 rounded-lg text-white">
              ...
              <li
                className="cursor-pointer font-semibold text-xs uppercase flex items-center gap-2 p-2.5 m-1 rounded-lg text-white hover:bg-purple-600 transition-colors duration-200"
                onClick={() => handleNavItemSelect(item.label)}
              >
                {React.createElement(item.icon, { className: "h-6 w-6" })}
                {item.label}
